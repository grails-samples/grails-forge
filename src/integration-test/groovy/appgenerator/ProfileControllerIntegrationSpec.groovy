...
class ProfileControllerIntegrationSpec extends Specification {
    ...
    void "test get profiles with curl"() {
        given:
        HttpResponse<List<String>> resp = client.toBlocking().exchange(HttpRequest.GET("/5.0.0/profiles").header("User-Agent", "curl"), List)

        expect:"The response is correct"

        resp.status() == HttpStatus.OK
        resp.header(HttpHeaders.CONTENT_TYPE) == "application/json;charset=UTF-8"
        resp.body().toSet() == ['vue', 'web', 'rest-api', 'react', 'angular'].toSet()
    }

    void "test get profiles without curl contains angular profile"() {
        when:
        HttpResponse<List<Map>> resp = client.toBlocking().exchange(HttpRequest.GET("/5.0.0/profiles"), Argument.of(List, Map))
        ...
        result.find { profile -> profile.name == 'angular' }.description == 'A profile for creating Grails applications with Angular 5+'
        ...
        result.find { profile -> profile.name == 'angular' }.features.find { feature -> feature.name == 'asset-pipeline' }.required == false
    ...
    @Ignore("profile angular2 removed in Grails 5")
    ...
    void "test get profiles without curl contains react profile"() {
        ...
        result.find { profile -> profile.name == 'react' }.description == 'A profile for creating a Grails application with a React frontend'
    ...
    void "test get profiles without curl contains web profile"() {
        ...
        result.find { profile -> profile.name == 'web' }.features.find { feature -> feature.name == 'asset-pipeline' }.defaultFeature == false
        ...
        result.find { profile -> profile.name == 'web' }.features.find { feature -> feature.name == 'asset-pipeline' }.required == true
    ...
    @Ignore("Profile webpack removed in Grails 5")
    ...
    void "test get features with curl"() {
        when:
        HttpResponse<List<String>> resp = client.toBlocking().exchange(HttpRequest.GET("/5.0.0/web/features").header("User-Agent", "curl"), Argument.of(List, String))

        then:"The response is correct"
        resp.status() == HttpStatus.OK
        resp.header(HttpHeaders.CONTENT_TYPE) == "application/json;charset=UTF-8"
        resp.body().toSet() == ['asset-pipeline', 'hibernate5', 'json-views', 'less-asset-pipeline', 'markup-views', 'mongodb', 'neo4j', 'rx-mongodb', 'gsp', 'geb2', 'events'].toSet()
    }

    void "test get features without curl"() {
        when:
        HttpResponse<List<Map>> resp = client.toBlocking().exchange(HttpRequest.GET("/5.0.0/web/features"), Argument.of(List, Map))
        ...
        result.find { feature -> feature.name == 'asset-pipeline'}.required == true
        ...
        result.find { feature -> feature.name == 'json-views'}.required == false
